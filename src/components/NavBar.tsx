...
function GradientTitle({
  ...
  return (
    <span className={'text-1.5rem font-500'} style={style}>
      {children}
    </span>
  );
}
  ...
  return (
    <div className="flex items-center justify-between container m-auto h-full w-90vw bg-transparent">
      ...
      <div className="flex items-center text-1.5rem">
        <GradientTitle
          rotate={config.titleRotate}
          start={config.colorStart}
          end={config.colorEnd}
        >
          PlantUML Platform React
        </GradientTitle>
        🤩
      </div>
      <div className="flex items-center justify-end w-fit ms-auto gap-4">
        <IconGithub
          ...
          onClick={() => {
            window.open(
              'https://github.com/Wadehl/plantuml-platform-react',
              '_blank'
            );
          }}
        />
        ...
        <Switch
          ...
          uncheckedIcon={<Sun className="text-gray-900" />}
